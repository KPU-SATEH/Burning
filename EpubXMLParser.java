...
public class EpubXMLParser extends XMLParser implements Runnable {
    private ArrayList<EpubDatas> mDataList; // 원하는 값을 EpubDatas객체에 저장해서 리스트 생성
    ...
    public void startParsing() {
        // 부모클래스의 getXMLParser을 이용해서 XmlPullParser객체를 받아옴
        XmlPullParser parser = getXMLParser("utf-8");

        if (parser == null) {
            mDataList = null;
            Log.d("EpubXMLParser", "Paser Object is null");
        }
        else {
                ...
                while(parserEvent != XmlPullParser.END_DOCUMENT) {
                    switch (parserEvent) {
                        ...
                        case XmlPullParser.TEXT:                // case4. XmlPullParser.TEXT
                            if (tagIdentifier == 1) {
                                epub = parser.getText().trim(); // getText()메소드: 데이터를 가져옴
                            }
                            else {
                                // 예외처리 !!!
                            }
